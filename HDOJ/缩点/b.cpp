...
// }
int dfn[N], low[N], dfs_clock, stk[N], top, scc, sccno[N], du[N], cost[N], mc[N];
bool in_stk[N];
vector<int> e[N];
void tarjan(int u) {
    in_stk[u] = 1;
...
void rmain() {
    ...
    fill(mc + 1, mc + n + 1, 1e9);
    for (int i = 1; i <= n; i++) scanf("%d", &cost[i]), e[i].clear();
    for (int i = 1; i <= m; i++) {
        int u, v;
        scanf("%d %d", &u, &v);
        e[u].push_back(v);
    ...
    for (int i = 1; i <= n; i++) for (int v : e[i]) if (sccno[i] != sccno[v]) du[sccno[v]]++;
    ll ans = 0;
    int cnt = 0;
    for (int i = 1; i <= scc; i++) if (du[i] == 0) {
        ans += mc[i];
        cnt++;
    } 
    printf("%d %lld\n", cnt, ans);
